...
class HighMixer(nn.Module):
    def __init__(self, dim, kernel_size=3, stride=1, padding=1):
        ...
        self.cnn_dim = self.cnn_in * 2
        self.pool_dim = self.pool_in * 2

        self.conv1 = nn.Conv2d(self.cnn_in, self.cnn_dim, kernel_size=1, stride=1, padding=0, bias=False)
        self.proj1 = nn.Conv2d(self.cnn_dim, self.cnn_dim, kernel_size=kernel_size, stride=stride, padding=padding, bias=False, groups=self.cnn_dim)
        self.mid_gelu1 = nn.GELU()
       
        self.Maxpool = nn.MaxPool2d(kernel_size, stride=stride, padding=padding)
        self.proj2 = nn.Conv2d(self.pool_in, self.pool_dim, kernel_size=1, stride=1, padding=0)
...
class Mixer(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., attention_head=1, pool_size=2):
        ...
        self.low_mixer = LowMixer(self.low_dim, num_heads=attention_head, qkv_bias=qkv_bias, attn_drop=attn_drop, pool_size=pool_size)

        self.conv_fuse = nn.Conv2d(self.low_dim + self.high_dim * 2, self.low_dim + self.high_dim * 2, kernel_size=3, stride=1, padding=1, bias=False, groups=self.low_dim + self.high_dim * 2)
...
class TransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = Mixer(dim=embed_dims)
